<template>
  <section 
    ...
    :class="[isInView ? 'bg-[#304838]' : 'bg-white']"
  >
    <div   class="max-w-[1440px] mx-auto px-6 h-96 ">
      <div class="relative z-10">
        ...
        >
          <span 
            class="inline-block transition-colors duration-1000 mt-36" 
      ...
      onEnter: () => {
        if (!hasAnimated.value) {
          ...
          gsap.fromTo(headingRef.value, 
            { y: 100, opacity: 0 },
            { y: 0, opacity: 1, duration: 3.0, ease: 'power4.out' }
          )

          // Background color change
          gsap.to(sectionRef.value, {
            backgroundColor: '#304838',
            duration: 1.5,
            ease: 'power2.inOut',
          })
